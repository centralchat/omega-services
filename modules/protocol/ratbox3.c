...
#define NAME        "Ratbox 3.0+"
#define VERSION     "$Id: ratbox3.c 2310 2012-01-30 01:59:42Z jer $"
#define AUTHOR      "Omega Team"
#define SUPPORTED   1

static int ratbox_startup();
static char	sid[20];
...
MODHEADER(NAME, VERSION, AUTHOR, MAKE_ABI(0,6,5),ratbox_startup, NULL);
...
static int ratbox_startup()
{
    ...
    {
        IRCd.ts6    = 1;
        alog (0, "Protocol requires TS6 Support.. Enabling\n");
    }

    addp_connect    (ratbox_connect);
    addp_ping       (ratbox_ping);
    addp_wallop     (ratbox_wallop);
    addp_add_user   (ratbox_nick);
    addp_join       (ratbox_join);
    addp_part       (ratbox_part);
    addp_xline      (ratbox_xline);
    addp_server     (ratbox_server);
    addp_numeric    (ratbox_numeric);
    addp_kill       (ratbox_kill);

    strlcpy (ProtocolInfo.name, "ratbox3", sizeof(ProtocolInfo.name));
    ProtocolInfo.ts		= 1;
...
void ratbox_numeric(User *target, int numeric, char *text) {
	send_line (":%s %d %s %s", (IRCd.ts6) ? Omega->me->sid : Omega->me->name, numeric, HasId(target) ? target->uid : target->nick, text);
}



static void ratbox_mode (Link *server, User *user, Channel *channel, char *modes, char *modeparams)
{
	char	*params[15];
	int		i;
...
void ratbox_server (Link *serv)
{
    // :42X SID ircd.botbay.net 2 5A1 :Botbay's Primary Server
    if (IRCd.ts6)
        send_line (":%s SID %s 2 %s :Omega Relay Server", CfgSettings.sid, serv->name, serv->sid);
}

void ratbox_wallop(char *source, char *msg)
{
    User    *u;
	...
	if ((strcasecmp(source, CfgSettings.servername)) == 0)
		send_line (":%s WALLOPS :%s", s_Guardian->uid, msg);
	else
		send_line (":%s WALLOPS :%s", u->uid, msg);
	return;
}

void ratbox_ping (char *source)
{
...
//		ircd_wallop ("Guardian", "End of Burst");
	...
	send_line (":%s PING %s :%s", CfgSettings.sid, CfgSettings.sid, source);
}

void ratbox_pong (char *source)
{
    if (sync_state == BURSTING)
    {
        Event ("BURST", 0, NULL);
...
//      ircd_wallop ("Guardian", "End of Burst");
    ...
    send_line (":%s PONG %s :%s", CfgSettings.sid, CfgSettings.sid, source);

    return;
}

void ratbox_nick (User* user)
{

	send_line (":%s UID %s 1 %lu %s %s %s %s %s :%s", user->serv->sid, user->nick,
    ...
    //    send_line (":%s RESV * %s :Reserved for Security Services", s_Guardian->uid, user->nick);
    }

}

void ratbox_join (User *u, Channel *c)
{
    if (!u || !c)
        return;

    if (u->service)
        send_line (":%s SJOIN %ld %s %s :@%s", u->serv->sid, (unsigned long) c->channelts, c->name, create_chanmode_string (c), (HasId(u) && (IRCd.ts6 == 1))? u->uid : u->nick);
    else
        send_line (":%s SJOIN %ld %s %s :%s", u->serv->sid, (unsigned long) c->channelts, c->name, create_chanmode_string (c), (HasId(u) && (IRCd.ts6 == 1))? u->uid : u->nick);
...
void ratbox_part (User *u, Channel *c, char *message)
{
    if (!c || !u)
        return;

    send_line (":%s PART %s :%s", u->uid, c->name, (message)? message : "");
    return;
}

/* [Debug 2] RECV: :69BAAABT6 KLINE * 0 test test.com :Testing */

void ratbox_xline (char *what, char *source, char *mask, int bantime, char *reason)
{
	switch (*what)
	{
		case 'G':
			send_line (":%s ENCAP * KLINE 0 * %s :%s", source, mask, reason);
			break;
		default:
			send_line (":%s ENCAP * KLINE 0 * %s :%s", source, mask, reason);
			break;
...
void ratbox_kill (User *user, char *reason)
{
	if (!user)
		return;
	if (!s_Guardian)
	...
	send_line (":%s KILL %s :%s", (HasId(s_Guardian) && (IRCd.ts6 == 1))? s_Guardian->uid : s_Guardian->nick, (HasId(user) && (IRCd.ts6 == 1))? user->uid : user->nick, (reason)? reason : "No Reason");
	return;
}
...
static void omega_mode_user (User *source, int argc, char **argv)
{
    ...
    ratbox_mode (NULL, source, NULL, argv[2], NULL);
    return;
}
...
static void omega_tmode     (User *source, int argc, char **argv)
{
	...
	if ((c = find_channel (argv[2])))
	{
    	...
    	ratbox_mode (NULL, source, c, argv[3], params);
		return;
	}
...
static void omega_invite (User *source, int argc, char **argv)
{
    ...
    if ((!source) || (argc < 3) || (!source->oper) || ((user = find_user (argv[1])) == NULL))
        return;

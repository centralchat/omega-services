...
#define NAME        "Plexus 3.0+"
#define VERSION     "$Id: plexus.c 1781 2010-02-15 14:34:53Z jer $"
#define AUTHOR      "Omega Team"
#define SUPPORTED   1

static int plexus_startup();
static char	sid[20];
...
MODHEADER(NAME, VERSION, AUTHOR, MAKE_ABI(0,6,5),plexus_startup, NULL);
...
static int plexus_startup()
{
    ...
    addp_connect    (plexus_connect);
    addp_ping       (plexus_ping);
    addp_wallop     (plexus_wallop);
    addp_add_user   (plexus_nick);
    addp_join       (plexus_join);
    addp_part       (plexus_part);
    addp_xline      (plexus_xline);
	addp_numeric	(plexus_numeric);
	
	
    strlcpy (ProtocolInfo.name, "plexus", sizeof(ProtocolInfo.name));
    ProtocolInfo.ts		= 1;
...
void plexus_numeric(User *target, int numeric, char *text) {
	send_line (":%s %d %s %s", (IRCd.ts6) ? Omega->me->sid : Omega->me->name, numeric, HasId(target) ? target->uid : target->nick, text);
}

static void plexus_mode (Link *server, User *user, Channel *channel, char *modes, char *modeparams)
{
	char	*params[15];
	int		i;
...
void plexus_wallop(char *source, char *msg)
{
    User    *u;
...
void plexus_nick (User* user)
{

	send_line (":%s UID %s 1 %lu %s %s %s %s %s 0 %s :%s", CfgSettings.sid, user->nick,
				(unsigned long) time(NULL), create_usermode_string(user), user->user, user->host, "127.0.0.1", user->uid, user->host, user->realname);

    if (user->registered)
        send_line (":%s ENCAP * SU %s :%s", CfgSettings.servername, user->nick, user->nick);

//    send_line (":%s RESV * %s :Reserved for Security Services", user->uid, user->nick);
    send_line (":%s RESV * %s :Reserved for Security Services", s_Guardian->uid, user->nick);

}

void plexus_join (User *u, Channel *c)
{
    if (!u || !c)
        return;

    send_line (":%s SJOIN %ld %s %s :@%s", CfgSettings.sid, (unsigned long) c->channelts, c->name, create_chanmode_string (c), (HasId(u) && (IRCd.ts6 == 1))? u->uid : u->nick);

//    AddToChannelU (u, c);
    return;
}

void plexus_part (User *u, Channel *c, char *message)
{
    if (!c || !u)
        return;
...
void plexus_xline (char *what, char *source, char *mask, int bantime, char *reason)
{
	switch (*what)
	{
		case 'G':
			send_line (":%s KLINE * 0 * %s :%s", source, mask, reason);
			break;
		default:
			send_line (":%s KLINE * 0 * %s :%s", source, mask, reason);
			break;
...
static void omega_uid	(Link *li, int argc, char **argv)
{
	...
	if ((user = AddUser (argv[1], argv[5], argv[6], argv[11], argv[0], argv[4], argv[8], NULL, argv[7])) == NULL) {
...
static void omega_mode_user (User *source, int argc, char **argv)
{
    ...
    plexus_mode (NULL, source, NULL, argv[2], NULL);
    return;
}
...
static void omega_tmode     (User *source, int argc, char **argv)
{
	...
	if ((c = find_channel (argv[2])))
	{
    	...
    	plexus_mode (NULL, source, c, argv[3], params);
		return;
	}

#include "stdinc.h"
#include "server.h"
#include "directircd.h"

#define NAME        "DirectIRCd"
#define VERSION     "$Id: $"
...
static int ratbox_startup()
{
    ...
    addp_mode	    (ratbox_mode);

	
    strlcpy (ProtocolInfo.name, "directircd", sizeof(ProtocolInfo.name));
    ProtocolInfo.ts		= 1;
...
void ratbox_mode(User *u, Channel *c, char *mode) {
       char *params[15], m[512], modes[100]; 
       int i, cnt;
       User *trg;
       ...
       if (!u || !c)
       		return;
	strlcpy(modes, params[0], sizeof(modes));
        for (cnt = 1; cnt < i; cnt++) {
		if ((trg = find_user (params[cnt])))
			strncat(m, (trg->uid)? trg->uid : trg->nick, 511 - strlen(m));
		else
			strncat(m, params[cnt], 511 - strlen(m));
 	}
	read_chanmode_string (c, modes, params);
	send_line(":%s TMODE %lu %s %s %s", (u->uid)? u->uid : u->nick,c->channelts,  c->name, modes, m);
}



static void ratbox_mode_ext (Link *server, User *user, Channel *channel, char *modes, char *modeparams)
{
	char	*params[15];
	int		i;
...
void ratbox_part (User *u, char *c, char *message)
{
    if (!c || !u)
        return;
...
static void omega_tmode     (User *source, int argc, char **argv)
{
	...
	if ((c = find_channel (argv[2])))
	{
    	...
    	ratbox_mode_ext (NULL, source, c, argv[3], params);
		return;
	}
...
static void omega_forcepart (User *source, int argc, char **argv)
{
    ...
    ratbox_part (user, argv[2], message);

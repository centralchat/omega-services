...
#define NAME        "Elemental-ircd"
#define VERSION     "$Id: $"
#define AUTHOR      "Niichan <shadowh511@gmail.com>"
#define SUPPORTED   1

static int elemental_startup();
static char	sid[20];
...
MODHEADER(NAME, VERSION, AUTHOR, MAKE_ABI(0,6,5), elemental_startup, NULL);
...
static int elemental_startup()
{
    ...
    addp_connect    (elemental_connect);
    addp_ping       (elemental_ping);
    addp_wallop     (elemental_wallop);
    addp_add_user   (elemental_nick);
    addp_join       (elemental_join);
    addp_xline      (elemental_xline);
	addp_numeric	(elemental_numeric);
...
void elemental_numeric(User *target, int numeric, char *text) {
	send_line (":%s %d %s %s", (IRCd.ts6) ? Omega->me->sid : Omega->me->name, numeric, HasId(target) ? target->uid : target->nick, text);
}



static void elemental_mode (Link *server, User *user, Channel *channel, char *modes, char *modeparams)
{
	char	*params[15];
	int		i;
...
void elemental_wallop(char *source, char *msg)
{
    User    *u;
...
void elemental_ping (char *source)
{
...
void elemental_join (User *u, Channel *c)
{
    if (!u || !c)
        return;
...
void elemental_part (User *u, char *c, char *message)
{
    if (!c || !u)
        return;
...
void elemental_xline (char *what, char *source, char *mask, int bantime, char *reason)
{
	User *u = NULL;
...
static void omega_mode_user (User *source, int argc, char **argv)
{
    ...
    elemental_mode (NULL, source, NULL, argv[2], NULL);
    return;
}
...
static void omega_tmode     (User *source, int argc, char **argv)
{
	...
	if ((c = find_channel (argv[2])))
	{
    	...
    	elemental_mode (NULL, source, c, argv[3], params);
		return;
	}
...
static void omega_forcepart (User *source, int argc, char **argv)
{
    ...
    elemental_part (user, argv[2], message);
